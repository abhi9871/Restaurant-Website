...
const CartModal = ({ onClose }) => {
  ...
  const addItemHandler = (e) => {
    const itemId = e.target.id;
    const item = cartCtx.items.find((meal) => meal.id === itemId);

    if (item.amount < 5) {
      cartCtx.increasedQuantity(itemId);
    }
  };

  useEffect(() => {
    // Check if all items are removed
    setIsMsgShown(cartCtx.items.length === 0);
  }, [cartCtx.items]);
        ...
        <Card>
          {isMsgShown ? (
            <Card.Body className="text-center fw-bold">
              <p>
                Your cart is feeling a bit lonely..😢. Why not add some
                delicious items?
              </p>
            </Card.Body>
          ) : (
